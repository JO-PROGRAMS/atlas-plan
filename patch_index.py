...
# 1. Background fix
html = re.sub(
    rbmxbackground:\s*url\('https://images\.unsplash\.com/[^']+'\)\s2center/cover\s2no-repeat;?",
    "background: linear-gradient(175deg, hsla(232, 78%, 24%, 1) 0%, hsla(210, 20%, 6%, 1) 68%);",
    html
)

# 2. Button removals based on explicit user prompt
...
html = re.sub(r'\.tc-actions\x.*?\}', '', html)
html = re.sub(r'\.tc-actions \.tc-btn\x.*?\}', '', html)

# 3. Tcard clipping issues fix
replace1 = ".tcsum {font-size:14px;color:rgba(255,255,255,.8);line-height:1.6;margin-top:8px;margin-bottom:12px;font-weight:400;width:100%;white-space:pre-wrap;display:block;}"
...
# Modify tcard container logic to NOT clip and align with flex
html = re.sub(
    r'\.tcard\{[^\}]*\min-height:180px;height:auto;[^\}]*\}',
    '''.tcard{
  width:100%;max-width:920px;margin:0 auto;display:flex;flex-direction:column;justify-content:flex-start;gap:12px;
  min-height:120px;height:auto;border-radius:24px;background:rgba(255,255,255,0.02);
  backdrop-filter:blur(32px) saturate(1.2);-webkit-backdrop-filter:blur(32px) saturate(1.2);
  border:1px solid rgba(255,255,255,0.1);padding:24px;position:relative;cursor:grab;transition:transform 0.3s cubic-bezier(0.2,0.8,0.2,1),box-shadow 0.3s cubic-bezier(0.2,0.8,0.2,1);
  box-shadow:inset 0 1px 0 rgba(255,255,255,0.15),inset 0 0 12px rgba(255,255,255,0.05),0 12px 32px rgba(0,0,0,0.3);
}''',
    html,
    count=1,
    flags=re.DOTALL
...
# 4. Buttery smooth FLIP draggingold_dragover = """  container.addEventListener('dragover',e=>{
    e.preventDefault();
...
# Increase the contrast/intensity of the ::before drag interactions for better refraction
html = html.replace(
    ...
    'background:conic-gradient(from cal`((¡Ù…È ´µ‘É…œµà¤€¬Ù…È ´µ‘É…œµä¤¤€¨€ÄàÁ‘•œ¤±É‰„ ÈÔÔ°ÄÀà°ÄÌÀ°¸àØ¤±É‰„ ÈÔÔ°ÄäÄ°ÄÀÔ°¸àØ¤±É‰„ ÈÔÌ°ÈĞÈ°ÄÈÀ°¸à¤±É‰„ ÄÌÀ°ÈĞØ°Äàä°¸àÈ¤±É‰„ ÄÈÔ°ÈÄÄ°ÈÔÈ°¸àØ¤±É‰„ ÄØÜ°ÄÌä°ÈÔÀ°¸àà¤±É‰„ ÈĞĞ°ÄÄĞ°ÄàÈ°¸àØ¤±É‰„ ÈÔÔ°ÄÀà°ÄÌÀ°¸àØ¤¤ìœ(¤()İ¥Ñ ½Á•¸ ¥¹‘•à¹¡Ñµ°œ°€Üœ°•¹½‘¥¹œôÕÑ˜´àœ¤…Ì˜è(€€€˜¹İÉ¥Ñ”¡¡Ñµ°¤()ÁÉ¥¹Ğ ‰A…Ñ …ÁÁ±¥•ÍÕ•ÍÍ™Õ±±äˆ¤